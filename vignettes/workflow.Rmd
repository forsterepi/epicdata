...
### 2.1) Variables that are in the data but not in the metadata (Sub-Module ID: VAR_DF.NOT.META)
...
| Activity               | Condition                          |
|------------------------|------------------------------------|
| active: Variant `NOTE` | option `remove.vars: no` (default) |
| active: Variant `RM`   | option `remove.vars: yes`          |
| inactive               | never                              |

#### State of the data

| Data characteristic | Before | After |
|---------------------|--------|-------|
| all.vars.character  | TRUE   | TRUE  |
|                     |        |       |
|                     |        |       |

#### User input

Requires [confirmation]{.underline} that variables should be missing, removed

### 2.2) Notice variables that are in meta but not in the data (Sub-Module ID: VAR_META.NOT.DF.NOTE)
...
### 2.3) Apply old.id (Sub-Module ID: VAR_OLD.ID)
...
Use `attr(,"problems")` for the result of `readr::parse_double()` to get the rows that would result in NA when parsing

## 10) Change data type (Module ID: TYPE)

Use `date::parse_date_time()` for dates and datetime (add timezone argument, how to handle locale necessary?)

Use lubri`date::hms()`, `lubridate::ms()`, `lubridate::hm()` (or functions from `hms` package directly?)

Use `readr::parse_double()` for numbers (add decimal.mark and grouping.mark as metadata fields)

Well, probably use `vctrs::vec_cast()` for all of them?

## Probably here: import variables from other datasets
...
Include a plyr::count() table of all changes based on .rules, so that we can easily evaluate if all changes made sense and that we did not forget anything.
