...
-   na.rules are applied before all other rules. Therefore, all other rules need to address the post NA processing values, i.e., the missing codes, NA only if na.touch: no, or the values resulting from na.replace; if you can address shortened NA codes for date,time,datetime, e.g. 9001 instead of 9001-01-01, is tbd (ideally yes); na.rules themselves are applied in order of occurrence of the variable in the YAML input. Therefore, if you include variables in your conditions which appear before the current one, use na.codes, etc., and if you address a variable after the current one, use NA even if na.touch: yes
