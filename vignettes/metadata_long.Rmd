...
Colons `:` are used to specify **key-value pairs**, i.e., an element of a certain name (key) as well as its corresponding value. Per line, only one key-value pair is specified. The basic syntax is `key: value`. **Always include a space after the colon.** Usually no quotation marks `""` are necessary when specifying values. Some exceptions are listed below. Almost all keys are pre-defined by `epicdata` to structure your metadata. The values usually must have a specific format, e.g., a number, or come from a list of allowed values. The values `true` and `false` will be transformed to `TRUE` and `FALSE` in R.

When to use quotation marks `""`:

- Quatation marks are used for specifying strings, i.e., character values, i.e., text. By default, you don't need to wrap strings in quotation marks.
- Use them, however, if you start a value with one of the following symbols: `-`, `?`, `!`, `:`, `,`, `#`, `&`, `%`, `*`, `` ` ``, `>`, `|`, `@`, because they have a special function in YAML and will throw an error or lead to unexpected results. If the symbol appears not at the start but later in the string, e.g., `Hey!`, you do not need quotation marks. Starting a string with the following symbols works without quotation marks as well: `.`, `_`, `<`. If, e.g., you work with a dataset that uses `.` and `-` for missing values, you need to specify `"-"` (in `to.na`), i.e., with quotation marks, but `.`, i.e., without quotation marks, works. Of course, wrapping `.` in quotation marks, i.e., `"."`, works as well.
- Use them, when a string contains (not only starting with) `[]` or `{}`, e.g., when specifing regular expressions. `()` do not need quatation marks.
- Use them, when the string contains `:`, i.e., a colon and a space, or `#`, i.e., a space and a hashtag. `:` will lead to an error because it is reserved for key-value pairs. `#` will treat everything afterwards as a comment (without throwing an error) and only part of your string is used in the metadata.
...
- Use them, when strings start with extra spaces, because otherwise, they will be removed.
- The same rules apply to keys and values, i.e., the part in front and after the colon `"key": "value"`
- If reading your metadata leads to a `Loading YAML metadata failed!` error or a `YAML parse error` with the line number of the problem's location, try wrapping it in quotation marks as your first debugging activity. Sometimes, the line above the line mentioned in the error message needs the quotation marks.
...
Dashes `-` are used to form **lists** or **arrays**. Even though the YAML language can be more complicated, we only use dashes for lists of values when you need to map multiple values to the same key. All values in the same list need to have the same indentation and all of them have a dash. For this, the two space indentation is replaced by one dash and one space (`- value`). In the example below, `million.cities` expects an array and, therefore, even though France only has one city with more than one million inhabitants, the value is specified as an array. Germany has four cities with more than one million inhabitants and each element is specified in a new line. Notice that the dash has the same indentation as `million.cities`, but due to the dash and the additional space, the values themselves (`Berlin`, `Hamburg`, etc.) are indented by two characters, as usual. `epicdata` usually expects the individual elements of an array to be strings, i.e., text. You will get an error message when, e.g., one of the elements is a number without any letters. You can force these values to be strings by wrapping them in quotation marks.
...
If strings are too long to keep them in a single line, e.g., for long questionnaire items, use the `>-` syntax to use multiple lines. As described in the code block below, for splitting a string across multiple lines, write `>-` after the key, and switch to a new line. The lines that contain the string need an indentation! In the final result, your lines will be separated by spaces, i.e., ``` "``This label is much too long to keep it in a single line, because that makes it hard to read.``" ```.

``` yaml
label: This label is much too long to keep it in a single line, because that makes it 
label2: >-
  This label is much too long to keep it
  in a single line, because that makes it 
  hard to read.
```

YAML files can include comments. Like in R, **comments** start with `#`. If they appear in a line after valid code, there must be a space in front of the `#`. In R Studio, comments are coloured differently.

``` yaml
countries:
# a valid comment in a seperate line
  France: # a valid comment after valid code (note the space in front of #)
    capital: Paris# not a valid comment since there is no space before the hashtag symbol
```

Don't include keys without value. Note, however, that the value does not need to be in the same line as it can consist of other key-value pairs on a lower layer, i.e., indented/moved to the right.

``` yaml
countries: # valid, because the value for country are all indented lines below
  Germany: # valid, because the value for Germany are all indented lines below
    capital: Berlin
    population.million: # invalid, because value is missing
    eu.member: true
  France: # invalid, because there are no indented lines below France
```

YAML code is not specified in R files but in special text files with file extension `.yml`. The `yml` files can be created by function `create_yaml()` and edited in R Studio or Positron. When using Positron together with the YAML extension by Red Hat, auto complete, YAML validation, and hover support are available. Please note that you use all software at your own risk. In `create_yaml()`, set `include_schema` to `TRUE` to the file with a JSON schema, which describes the format that the YAML needs to have. When editing the YAML file in, press Ctrl+Space to open the suggestions menu. Errors in Positron's Problems tab might be difficult to understand. Try creating the metadata object with `epicdata::metadata()` for better error messages. Advanced YAML editing features are not available in R Studio. 
...
- `import`: Make variables from related datasets available for inclusion in rules
...
- `id.list` (boolean, i.e., `true`, `false`): Indicate if a list with valid IDs is provided. An `id.list` makes sense if users are unsure how to specify an `id.pattern` with regular expressions, if the `id.pattern` is not sufficient to differentiate between valid and invalid IDs, i.e., some IDs with the specified pattern are valid and some are not, or if a list of all used IDs is easily available. Should not (Cannot??) be combined with `id.pattern`.

- `consent` (boolean, i.e., `true`, `false`): Indicate if informed consent will be checked. Can only be `true`, if an `id.var` has been specified.
...
- `double.entry` (boolean, i.e., `true`, `false`): Non-digital data has been entered multple times (usually twice) to avoid errors during data entry. This option activates sub-module DUP_COMPARE.

- `remove.vars` (boolean, i.e., `true`, `false`): Indicate if variables in the data that are missing from the metadata should be removed
...
- `touch.na` (boolean, i.e., `true`, `false`): Indicate if NAs should be changed to missing codes

- `to.factor` (boolean, i.e., `true`, `false`):
...
- `label.eng` (Text): **Change to more flexible language support** If the `label` is not in English, enter its English translation here for use in the data dictionary. `label.eng` can only be used if `label` is specified as well.

- `to.na` or `na.to` (text):

- `touch.na` or `na.touch` (boolean, i.e., `true`, `false`): If `false`, all `NA`s, after applying `to.na`, will remain as `NA`. If `true`, `NA`s are processed as usual. Defaults to the global option `touch.na`, or, if the global options has not been specified, to `true`.

- `na.else` (a missing code): The missing code (as defined in component `na.codes`) specified here is used to replace `NA`s if no rule in `na.rules` applies. Must be specified always, apart from situations when the variable has no `NA`s or when `touch.na` is set to `false`. **Will become global option and its function for individual variables is included into na.rules.**
...
- `cats.eng` (): **Change to more flexible language support**
...
- `to.factor` (boolean, i.e., `true`, `false`): If `true`, transforms the variable to factor.

- `factor.name` or `name.factor` (): If `to.factor` is `true` or not specified, a new variable is created and turned to factor while this variable stays integer. Provide the name of the new factor variable.

## **Numeric (num) variables**

- `unit` (text): Name the unit of the values
...
- Applies to na.rules, dict.rules, cat.rules, limit.rules, contradictions

- When rules are applied, variables are in their corresponding type already, i.e., after changing them from type character to their actual type, e.g., numeric. Therefore, constants in the rules need to be handled accordingly, e.g., putting strings in quotation marks

- NA can be written as NA, without quotation marks, just like in R

- na.rules are applied before all other rules. Therefore, all other rules need to address the post NA processing values, i.e., the missing codes, NA only if na.touch: false, or the values resulting from na.replace; if you can address shortened NA codes for date,time,datetime, e.g. 9001 instead of 9001-01-01, is tbd (ideally yes); na.rules themselves are applied in order of occurrence of the variable in the YAML input. Therefore, if you include variables in your conditions which appear before the current one, use na.codes, etc., and if you address a variable after the current one, use NA even if na.touch: true

- Rules follow in general the format of formulas in dplyr::case_when: they are two-sided formulas with a condition on the left hand side and a value on the right hand side.
...
**It must be checked if after applying the aforementioned conversion processes, there are actually multiple identical na.codes, e.g., imagine there is a missing code 9001 and 9001-01-01, with different meaning, then a transformed 9001 would be the same as 9001-01-01. However, for this we ideally need to check, if 9001 is actually used in any date variable. Maybe: 0) check if duplicated codes have been specified in na.codes; 1) get list of na.codes used by variable with their description provided in component na.codes; 2) for date, datetime, time, transform codes as discussed above; 3) check if all codes of all variables are in the required format for that variable; 4) if there are identical codes with different descriptions =\> error,**

**Also, we should allow negative numbers for codes. Unquoted numbers as keys (9001: missing) will end up in the "yaml_keys" attribute and the name in the list is an empty string ("")**
...
- `to.factor` (boolean, i.e., `true`, `false`): Sets the default for all variables in the set to the specified value. Therefore, if `true` is specified, `to.factor` of all variables in the mc.set is set to `true` as well. You can overwrite the value for individual variables by setting their value in `var.list` back to `false`.
...
See vignette multiple_datasets
