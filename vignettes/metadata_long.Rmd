...
Colons `:` are used to specify **key-value pairs**, i.e., an element of a certain name (key) as well as its corresponding value. Per line, only one key-value pair is specified. The basic syntax is `key: value`. Always include a space after the colon. Usually no quotation marks `""` are necessary when specifying values. However, use them when you start the value with a special symbol (non-letter and non-number) apart from dot `.` and underscore `_`, or when you want to force a value to be a string. (The `calling.code` would be read as the positive number `33` without quotation marks.) Almost all keys are pre-defined by `epicdata` to structure your metadata. The values usually must have a specific format, e.g. a number, or come from a list of allowed values. The values `yes` and `no` (similarly to `true` and `false`) will be transformed to `TRUE` and `FALSE` in R.
...
-   `changes`: Individual changes to certain cells, e.g., because a participant reported an error in the questionnaire comment.
