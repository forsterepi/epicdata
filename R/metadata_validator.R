...
var.list.validator <- function(value) {
  ...
  if (
    ...
    return(
      "must be a list of lists, must have unique variable names, must have
  ...
  keys.to.na <- c(
    ...
    "na.to",
    "to.na.default.group",
    "to.na.default.option",
    "to.na.final"
  )
  keys.touch.na <- c(
    "touch.na",
    "na.touch",
    "touch.na.default.group",
    "touch.na.default.option",
    "touch.na.final"
  )
  keys.na.else <- c(
    "na.else",
    "else.na",
    "na.else.default.group",
    "na.else.final"
  )
  keys.na.rules <- c(
    "na.rules",
    "rules.na",
    "na.rules.default.group",
    "na.rules.final"
  )
  keys.na.switch <- c(
    "na.switch",
    "switch.na",
    "na.switch.default.group",
    "na.switch.final"
  )
  keys.na.keep <- c(
    "na.keep",
    "keep.na",
    "na.keep.default.group",
    "na.keep.final"
  )
  keys.na.replace <- c(
    "na.replace",
    "replace.na",
    "na.replace.default.group",
    "na.replace.final"
  )
  keys.group <- c("group")
  keys.sub <- c("sub")
  ...
  keys.cats.eng <- c(
    "cats.eng",
    "eng.cats",
    "cats.eng.default.group",
    "cats.eng.final"
  )
  keys.to.factor <- c(
    "to.factor",
    "to.factor.default.group",
    "to.factor.default.option",
    "to.factor.final"
  )
  ...
  keys.date.format <- c(
    ...
    "format.date",
    "format",
    "date.format.default.group",
    "date.format.default.option",
    "date.format.final"
  )
  keys.time.format <- c(
    "time.format",
  ...
  for (i in seq_along(value)) {
    if (
      ...
      return(paste0(
        "contains invalid keys in variable '",
        names(value)[i],
        "', namely: ",
        invalid
      ))
...
var.groups.validator <- function(value) {
  ...
  if (
    ...
    return(
      "must be a list of lists, must have unique group names, must have
  ...
  for (i in seq_along(value)) {
    if (
      ...
      invalid <- names(value[[i]]) %>%
        ...
        stringi::stri_c(collapse = ", ")
      return(paste0(
        "contains invalid keys in group '",
        names(value)[i],
        "', namely: ",
        invalid
      ))
