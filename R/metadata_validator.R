...
var.list.validator <- function(value) {
  ...
  keys.to.na <- c(
    "to.na", "na.to", "to.na.default.group", "to.na.default.option",
    "to.na.final"
  )
  keys.touch.na <- c(
    "touch.na", "na.touch", "touch.na.default.group",
    "touch.na.default.option", "touch.na.final"
  )
  keys.na.else <- c("na.else", "else.na", "na.else.default.group", "na.else.final")
  keys.na.rules <- c(
    "na.rules", "rules.na", "na.rules.default.group",
    "na.rules.final"
  )
  keys.na.switch <- c(
    "na.switch", "switch.na", "na.switch.default.group",
    "na.switch.final"
  )
  keys.na.keep <- c("na.keep", "keep.na", "na.keep.default.group", "na.keep.final")
  keys.na.replace <- c(
    "na.replace", "replace.na", "na.replace.default.group",
    "na.replace.final"
  )
  keys.group <- c("group")
  keys.sub <- c("sub")
  ...
  keys.to.factor <- c(
    "to.factor", "to.factor.default.group",
  ...
  keys.time.format <- c(
    "time.format", "format.time", "format",
    "time.format.default.group", "time.format.default.option",
    "time.format.final"
  )
  keys.datetime.format <- c(
    "datetime.format", "format.datetime", "format",
    "datetime.format.default.group", "datetime.format.default.option",
    "datetime.format.final"
  )
  key.list <- c(
    keys.var.name, keys.type, keys.old.id, keys.label,
    keys.label.eng, keys.to.na, keys.touch.na, keys.na.else,
    keys.na.rules, keys.na.switch, keys.na.keep, keys.na.replace,
    keys.group, keys.sub, keys.new, keys.ops, keys.dict, keys.cats,
    keys.cats.eng, keys.to.factor, keys.factor.name, keys.from,
    keys.from.exclude, keys.to, keys.to.exclude, keys.date.format,
    keys.time.format, keys.datetime.format
  )

  # Loop over all variables
  for (i in seq_along(value)) {
    if (!checkmate::test_subset(names(value[[i]]),
      empty.ok = FALSE,
      choices = key.list
    )) {
      ...
      return(paste0(
        "contains invalid keys in variable '", names(value)[i],
        "', namely: ", invalid
      ))
    }

    if (!checkmate::test_logical(value[[i]][["touch.na"]],
      len = 1,
      any.missing = FALSE, null.ok = TRUE
    )) {
      return(paste0(
        "has touch.na (or na.touch) keys in the wrong format in variable '",
        names(value)[i], "'"
      ))
    }
    if (!checkmate::test_logical(value[[i]][["na.touch"]],
      len = 1,
      any.missing = FALSE, null.ok = TRUE
    )) {
      return(paste0(
...
var.groups.validator <- function(value) {
  ...
  keys.limit.rules <- c(
    "limit.rules", "limits.rules", "rule.limits",
    "rules.limits"
  )
  key.list <- c(
    keys.group.name, keys.group.label, keys.group.label.eng,
    keys.mc.exclusive,
  ...
  for (i in seq_along(value)) {
    if (!checkmate::test_subset(names(value[[i]]),
      ...
      invalid <- names(value[[i]]) %>%
        ...
        stringi::stri_c(collapse = ", ")
      return(paste0(
        "contains invalid keys in group '", names(value)[i],
        "', namely: ", invalid
      ))
    }
    ...
    if (!checkmate::test_logical(value[[i]][["touch.na"]],
      ...
      return(paste0(
        "has touch.na (or na.touch) keys in the wrong format in group '",
        names(value)[i], "'"
      ))
    }
    if (!checkmate::test_logical(value[[i]][["na.touch"]],
...
validator.metadata.default <- function(self, current_var) {
  ...
  ## .default.option
  if (!identical(
    self@var.list[[current_var]][["touch.na.default.option"]],
    self@touch.na
  )) {
    return(paste0(
      "Do not change @var.list$", current_var,
  ...
  if (is.null(self@var.list[[current_var]][["group"]])) {
    ...
    if (!identical(
      self@var.list[[current_var]][["touch.na.default.group"]],
