...
validate_metadata <- function(x) {
  ...
  checkmate::assert_subset(names(x), c("main","missing_codes","missing_rules","contras","mc_sets","string_dict"), add = coll)
  ...
  checkmate::assert_subset(colnames(x[["missing_codes"]]), c("class_codes","value_codes","label_codes"), add = coll)

  checkmate::assert_data_frame(x[["missing_rules"]], ncols = 3, col.names = "named", add = coll)
  checkmate::assert_subset(colnames(x[["missing_rules"]]), c("id_rules","var_rules","value_rules"), add = coll)
  ...
  checkmate::assert_subset(colnames(x[["mc_sets"]]), c("id_mc","vars_mc"), add = coll)

  checkmate::assert_data_frame(x[["string_dict"]], ncols = 4, col.names = "named", add = coll)
  checkmate::assert_subset(colnames(x[["string_dict"]]), c("id_dict","var_dict","value_dict","value_dict_eng"), add = coll)
...
empty_metadata <- function() {
  ...
  # missing_codes
  missing_codes_vars <- c("class_codes","value_codes","label_codes")
  ...
  # missing_rules
  missing_rules_vars <- c("id_rules","var_rules","value_rules")
  ...
  # string_dict
  string_dict_vars <- c("id_dict","var_dict","value_dict","value_dict_eng")
  x[[6]] <- matrix(c("d0",rep("NA",length(string_dict_vars) - 1)), nrow = 1, ncol = length(string_dict_vars)) %>%
    as.data.frame() %>%
    magrittr::set_colnames(string_dict_vars)
  names(x)[6] <- "string_dict"
...
print.epic_metadata <- function(x, ...) {
  ...
  cat("Dictionary Entries for String Variables: ",nrow(x$string_dict),"\n")
}
...
load_metadata <- function(path) {
  ...
  metadata$string_dict %<>% dplyr::mutate(dplyr::across(tidyselect::where(is.character), stringr::str_trim))
